...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        val newTree = LemonTree()
        if(lemonadeState == SELECT){
            lemonadeState = SQUEEZE
            lemonSize = newTree.pick()
            squeezeCount = 0
        }
    ...
    private fun setViewElements() {
        ...
        when(lemonadeState){
            SELECT -> {
                submitText = getString(R.string.lemon_select)
                lemonImage!!.setImageResource(R.drawable.lemon_tree)
            }
            SQUEEZE -> {
                submitText = getString(R.string.lemon_squeeze)
                lemonImage!!.setImageResource(R.drawable.lemon_squeeze)
            }
            DRINK -> {
                submitText = getString(R.string.lemon_drink)
                lemonImage!!.setImageResource(R.drawable.lemon_drink)
            }
            else -> {
                submitText = getString(R.string.lemon_empty_glass)
                lemonImage!!.setImageResource(R.drawable.lemon_restart)
            }
        }
        textAction.text = submitText
